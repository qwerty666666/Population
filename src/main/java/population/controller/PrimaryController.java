...
public class PrimaryController extends AbstractController {
    private final ObservableList<State> mStates = FXCollections.observableList(new ArrayList(),
    ...
    public TabPane mCalculationsTabPane;
    public Tab mResultChartTab;
    // TODO remove in view
    public TabPane mMainTabPane;
    public ParametricPortraitTabController mParametricPortraitTabController;
    ...
    private Callback<TableColumn<Transition, Number>, TableCell<Transition, Number>> typeCell() {
        return ChoiceBoxTableCell.forTableColumn(new StringConverter<Number>() {
            @Override
            public String toString(Number object) {
                return TransitionType.getName((int) object);
            }
    ...
    public void openTaskFromFile(String path) {
        File file = new File(path);
        if (file == null) {
            return;
        }
        Task task = null;//TaskParser.parse(file);
        if (task == null) {
            return;
        }
        setTask(task);
    ...
    private HashMap<String, String> buildSettings() {
        ...
        mTaskSettings
                .put(Task.Keys.SCALE, mResultsTablePrecisionField.getText());
...
//        if (settings.containsKey(Task.Keys.SCALE)) {
//            mResultsTablePrecisionField.setText(settings.get(Task.Keys.SCALE));
//        }
    }
    ...
    public void setControlsDisable(boolean value) {
...
//        mResultsTable.setPlaceholder(new Rectangle());
    ...
    public void calculate() {
        ...
        //task.setScale(mResultsTablePrecision);
        calculateTask(task);
    }
...
//        mParametricPortraitTabController.clearEnvironment();
    ...
    public void applyResultsTablePrecision() {
        ...
        /*if (mResultsTablePrecision == 0) {
            mResultsTablePrecision = mHigherAccuracy.isSelected() ? Calculator.HIGHER_ACCURACY_SCALE : Utils.MAX_PRECISION;
        }*/
        refreshResultsTable();
    }
